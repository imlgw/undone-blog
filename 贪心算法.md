...
```
### 解法一
![](https://i.loli.net/2021/01/17/73Cuvhx1KES5M2n.png)
货仓位置为$A[i]$，修建点为$x$，那么货仓到$x$点的距离之和为：
$$
\begin{aligned}
f(x) &= \sum_{i=0}^{n-1}|A[i]-x| \\
&= |A_0-x|+|A_1-x|+..+|A_{n-2}-x|+|A_{n-1}-x| \\
&= (|A_0-x|+|A_{n-1}-x|) + (|A_1-x|+|A_{n-2}-x|) + ... + (|A_{mid-1}-x|+|A_{mid+1}-x|) \\
&\geq(A_{n-1}-A_0) + (A_{n-2}-A_1) +...+ (A_{mid+1}-A_{mid-1})
